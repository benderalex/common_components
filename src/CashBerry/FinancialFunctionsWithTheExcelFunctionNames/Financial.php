...
/**
 * https://www.phpclasses.org/package/892-PHP-Financial-functions-with-the-Excel-function-names-.html#view_files/files/3028
...
class Financial
{
     ...
     * iterations to solve
     */
    protected const FINANCIAL_ACCURACY = 1.0e-6;
    protected const FINANCIAL_MAX_ITERATIONS = 100;
    protected const FINANCIAL_SECS_PER_DAY = 24 * 60 * 60;
    protected const FINANCIAL_HALF_SEC = 0.5 / self::FINANCIAL_SECS_PER_DAY;
     ...
     * date difference: each month 30 days, within a month serial delta
     */
    protected const FINANCIAL_BASIS_MSRB_30_360 = 0;
    protected const FINANCIAL_BASIS_ACT_ACT = 1;
    protected const FINANCIAL_BASIS_ACT_360 = 2;
    protected const FINANCIAL_BASIS_ACT_365 = 3;
    protected const FINANCIAL_BASIS_30E_360 = 4;
    protected const FINANCIAL_BASIS_30Ep_360 = 5;
    ...
    {
        return (($basis >= self::FINANCIAL_BASIS_MSRB_30_360) && ($basis <= self::FINANCIAL_BASIS_30Ep_360));
    }
     ...
     *
     */
    function DISC($settlement, $maturity, $pr, $redemption, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        ...
        switch ($basis) {
            case self::FINANCIAL_BASIS_MSRB_30_360: // US(NASD) 30/360
                $dsm = $this->Thirty360USyearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_ACT: // Actual days/actual days
                $dsm = $this->ActualActualyearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_360: // Actual days/360
                $dsm = $this->Actual360yearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_365: // Actual days/365
                $dsm = $this->Actual365yearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_30E_360: // European 30/360
     ...
     *
     */
    function RECEIVED($settlement, $maturity, $investment, $discount, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
        ...
        switch ($basis) {
            case self::FINANCIAL_BASIS_MSRB_30_360: // US(NASD) 30/360
                $dsm = $this->Thirty360USyearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_ACT: // Actual/actual
                $dsm = $this->ActualActualyearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_360: // Actual/360
                $dsm = $this->Actual360yearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_365: // Actual/365
                $dsm = $this->Actual365yearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_30E_360: // European 30/360
                $dsm = $this->Thirty360EUyearFraction($settlement, $maturity);
     ...
     *
     */
    function INTRATE($settlement, $maturity, $investment, $redemption, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
        ...
        switch ($basis) {
            ...
            case self::FINANCIAL_BASIS_ACT_ACT: // Actual/actual
                $dsm = $this->ActualActualyearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_365: // Actual/360
                $dsm = $this->Actual360yearFraction($settlement, $maturity);
                break;
            case self::FINANCIAL_BASIS_ACT_365: // Actual/365
                $dsm = $this->Actual365yearFraction($settlement, $maturity);
    ...
    function RATE($nper, $pmt, $pv, $fv = 0.0, $type = 0, $guess = 0.1)
    {
        ...
        if (abs($rate) < self::FINANCIAL_ACCURACY) {
        ...
        while ((abs($y0 - $y1) > self::FINANCIAL_ACCURACY) && ($i < self::FINANCIAL_MAX_ITERATIONS))
        {
     ...
     *
     */
    function YIELD($settlement, $maturity, $rate, $pr, $redemption, $frequency, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
     ...
     */
    function PRICEDISC($settlement, $maturity, $discount, $redemption, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
     ...
     **/
    function YIELDDISC($settlement, $maturity, $pr, $redemption, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
        if (($pr <= 0) || ($redemption <= 0)) return null;
        if ($settlement >= $maturity) return null;
     ...
     *
     */
    function COUPNUM($settlement, $maturity, $frequency, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
        if (!$this->_is_valid_frequency($frequency)) return null;
        if ($settlement >= $maturity) return null;
     ...
     *
     */
    function COUPDAYBS($settlement, $maturity, $frequency, $basis = self::FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) return null;
        if (!$this->_is_valid_frequency($frequency)) return null;
        if ($settlement >= $maturity) return null;
